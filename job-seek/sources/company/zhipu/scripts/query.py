...
智谱AI 使用飞书招聘页面，为 JS 动态渲染 SPA，需使用 Crawl4AI 渲染后提取。
URL: https://zhipu-ai.jobs.feishu.cn/index/position/list

用法:
  python query.py --keyword "Agent"
...
def parse_jobs_from_markdown(markdown, keyword):
    ...
    for line in lines:
        ...
        if is_job and re.match(r'^[\u4e00-\u9fa5A-Z]', line):
            ...
            current_job = {
                "id": "", "title": line, "company": "智谱AI",
                "location": "", "description": "", "requirements": "",
                "salary": "", "url": "", "source": "zhipu",
                "publishDate": "", "jobType": "",
            }
...
def main():
    parser = argparse.ArgumentParser(description="智谱AI招聘职位查询")
    ...
    try:
        ...
        print(f"[zhipu] 共 {len(jobs)} 条结果", file=sys.stderr)
    except Exception as e:
        print(f"[zhipu] 错误: {e}", file=sys.stderr)
        print("[]")
        sys.exit(1)
